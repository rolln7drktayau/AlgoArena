...
class RunManager:
    ...
    async def execute_run(self, run_id: str, run_request: RunRequest, websocket: WebSocket) -> RunRecord:
        ...
        step_timeout_sec = _env_float("ALGOARENA_ALGORITHM_STEP_TIMEOUT_SEC") or 30.0
        selected_algorithms = (
        ...
        for algo_cfg in selected_algorithms:
            ...
            run_record.algorithms[algo_cfg.id] = AlgorithmRunRecord(
                algorithm_id=algo_cfg.id,
                name=algo_cfg.label or algo_cfg.name,
            )
    ...
    def export_latex(self, run_id: str) -> str:
        ...
        rows = [
            "\\begin{table}[t]",
            "\\centering",
            "\\caption{AlgoArena benchmark results.}",
            "\\label{tab:algoarena-results}",
            "\\begin{tabular}{lrrrrr}",
            "\\hline",
            "Algorithm & HV & IGD & IGD+ & Epsilon & Time (s) \\\\",
            "\\hline",
        ]
        for row in leaderboard:
            rows.append(
                f"{self._latex_escape(row['algorithm_name'])} & "
                f"{self._fmt(row.get('hv'))} & "
                f"{self._fmt(row.get('igd'))} & "
                f"{self._fmt(row.get('igd_plus'))} & "
                f"{self._fmt(row.get('epsilon'))} & "
                f"{self._fmt(row.get('elapsed_sec'))} \\\\"
            )
        rows.extend(
            [
                "\\hline",
                ...
                "\\end{table}",
                "",
                "% Generated by AlgoArena. Add confidence intervals/statistical tests when repetitions are available.",
            ]
        )
        return "\n".join(rows)
    ...
    def export_statistics(self, run_id: str, metric: str = "hv") -> dict[str, Any]:
        ...
        for algo in run_record.algorithms.values():
            values = [
